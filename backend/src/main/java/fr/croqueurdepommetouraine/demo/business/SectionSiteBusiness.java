...
public class SectionSiteBusiness {
    ...
    private void checkUnique(SectionSiteEntity newSection) {
        if (SectionRepository.existsByPath(newSection.getPath())) {
            throw new RequeteIncorrect("A section with the same path already exists.");
        }
    }
    ...
    private void gestionParent(SectionSiteEntity section) {
        if (section.getParent() != null) {
            ...
            if (parent.isEmpty()) {
                throw new NotFoundException("Parent section with id " + section.getParent().getIdSection() + " not found.");
            }
    ...
    private void makeSectionValid(SectionSiteEntity section) {
        if (section.getPath() == null || section.getPath().isBlank()) {
            throw new RequeteIncorrect("Section path cannot be null or blank.");
        }
        if (section.getParent() != null) {
            if (section.getParent().getParent() != null) {
                throw new RequeteIncorrect("Le parent de la section ne peut pas avoir de parent");
            }
        }
    ...
    public SectionDAO getSectionByIdWithAuth(Long id, String authHeader) {
        ...
        if (!toolsAuthorisationEndPoint.CanReadThisSection(authHeader, id)) {
            throw new AccesInterditException("Pas les droits necessaires pour acceder a la section");
        }
